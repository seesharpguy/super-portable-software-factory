...
import { parseDecisionExtras, type Jev, type JevOption } from "../core/jev.ts";
 ...
 *  2. this loop has not already escalated (at most ONE rung per loop);
 *  3. tiering is enabled and actually routed `role` this run (a role
 *     tiering never touched has no rung to climb from — its own `model:`
 *     stands, as `resolveTiering` promises);
 *  4. the next rung up exists and is at or below `maxTier` on the ladder;
 *  5. that rung is usable by the same rule `resolveTiering` applies
...
export function nextRung(input: {
  ...
  const { cfg, resolution, role, maxTier, servedOllamaTags, alreadyEscalated } = input;
...
export interface FailedRound {
  ...
  phase: string;
  /** One line per failure/blocking item — what `history` keeps for later rounds. */
  brief: string[];
  /** The full detail of THIS round (clipped outputs, findings) — sent only for the latest round. */
...
export function createLoopControl(
  run: Run,
  opts: { loop: "fix" | "revise"; role: string; subject: string; max: number },
): LoopControl | null {
  ...
  const history: Array<{ round: number; phase: string; failures: string[]; action: LoopControlChoice }> = [];
  ...
  return {
    async afterFailedRound(round: FailedRound): Promise<LoopControlChoice> {
      ...
      const rung = nextRung({ cfg: run.cfg, resolution: run.tiering, role: opts.role, maxTier, servedOllamaTags, alreadyEscalated: escalated !== null });
      ...
      history.push({ round: round.round, phase: round.phase, failures: round.brief, action: choice });
      return choice;
    },
    restore(): void {
      if (escalated !== null) run.tiering = original;
    },
  };
}
