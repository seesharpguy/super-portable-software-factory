/**
 * SF visualizer server — JSON API over a target repo's sf.db, plus the
 ...
 *   bun run server/index.ts --db /path/to/repo/adws/adw_data/sf.db
 *   SF_DB=/path/to/sf.db PORT=4600 bun run server/index.ts
 */
import { existsSync, statSync } from "node:fs";
import { join, resolve, sep } from "node:path";
import { SfDb, resolveDbPath } from "./db.ts";
...
try {
  ...
  console.error(`[sf] ${(error as Error).message}`);
  process.exit(1);
}
...
function safely(
  ...
  return async (req) => {
    try {
      ...
      console.error(`[sf] ${req.method} ${new URL(req.url).pathname}:`, error);
...
async function serveStatic(req: Request): Promise<Response> {
  ...
  if (!existsSync(DIST_DIR)) {
    return new Response(
      `SF visualizer API is running on :${PORT}.\n\n` +
...
console.log(`[sf] visualizer api  http://localhost:${server.port}`);
console.log(`[sf] db              ${db.path}  [journal_mode=${db.journalMode}]`);
console.log(
  existsSync(DIST_DIR)
    ? `[sf] serving ui from  ${DIST_DIR}`
    : `[sf] no ./dist — use "bun run dev" for the Vite dev server on :4601`,
);
