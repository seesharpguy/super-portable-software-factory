/**
 * `sf eject` — copy the installed CLI's own compiled engine (`dist/core/`,
 * `dist/chains/`) out to disk for reference or hand-editing. There is no
 * wiring back in: `.sf/` config can override the roster and prompts, but an
 * engine-level change (a new gate, a new envelope type, a modified phase
 * primitive) has no config surface, by design — see
 * `authoring_chains.md`. This exists so `sf migrate`'s report has something
...
export function ejectCommand(argv: string[]): number {
  ...
  const destRoot = options["target"] ? path.resolve(options["target"]) : path.join(anchor.repo_root, ".sf", "engine");
  ...
  writeFileSync(
    ...
    `# Ejected engine — @grateful8/sf v${pkg.version}\n\n` +
      ...
      `wired into any \`sf\` command**: editing these files changes nothing about how \`sf\` itself runs.\n` +
      ...
      `and why it isn't a \`.sf/\` override.\n`,
  );
