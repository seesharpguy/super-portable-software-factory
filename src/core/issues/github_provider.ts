...
/**
 * `listPrComments`'s pagination bound, per endpoint — deliberately separate
 * from `MAX_LIST_PAGES`: unlike `listComments` (a single un-paginated
 * 100-item fetch, oldest-first, acceptable for an issue thread), a PR review
 * thread is exactly the place a truncated request would silently drop the
 * corrections this feature exists to read, so this one DOES paginate, capped
 * the same defensive way `listByLabel` is.
 */
...
const LABEL_META: Record<WatchState, { color: string; description: string }> = {
  ...
  feedback: { color: "0e8a16", description: "add this after leaving corrections as PR comments — spf will rebuild honoring them" },
...
export class GitHubProvider implements IssueProvider, CodeHostProvider, IssueAuthoringProvider {
  ...
  /**
   * Pages a `GET` endpoint whose items come back oldest-first with a plain
   * `per_page`/`page` cursor — the same shape `listByLabel` walks, extracted
   * here because `listPrComments` below needs the identical loop against
   * THREE different endpoints. Capped at `MAX_PR_COMMENT_PAGES`, with the
   * same loud-warning-not-silent-truncation rule as `listByLabel`.
   */
  private async ghPaged<T>(pathWithoutPage: string, label: string): Promise<T[]> {
    const results: T[] = [];
    for (let page = 1; page <= MAX_PR_COMMENT_PAGES; page++) {
      const sep = pathWithoutPage.includes("?") ? "&" : "?";
      const raw = await this.gh<T[]>(`${pathWithoutPage}${sep}per_page=100&page=${page}`);
      results.push(...raw);
      if (raw.length < 100) return results;
      if (page === MAX_PR_COMMENT_PAGES) {
        console.error(
          `spf watch: ${label} hit the ${MAX_PR_COMMENT_PAGES}-page (${MAX_PR_COMMENT_PAGES * 100}-item) cap — ` +
            `older entries past this cap are invisible this tick`,
        );
      }
    }
    return results;
  ...
  /**
   * Merges three GitHub surfaces into one oldest-first `PrComment[]` — see
   * `CodeHostProvider.listPrComments`'s own doc comment for why this exists.
   * (1) PR-level comments: literally the same endpoint `fetchComments` calls
   * for an issue, since a PR IS an issue on GitHub. (2) Inline/diff review
   * comments, which carry `path`/`line`. (3) Reviews — the only surface that
   * carries a decline/approve `state`, plus that review's own top-level body.
   * The three come back independently sorted ascending by their own
   * timestamp field; merge-sorted here into one thread. The hidden marker
   * comment (see `MARKER_RE`) is filtered out of (1) only — it can never
   * appear in (2)/(3), which are PR-review-specific surfaces `writeMarker`
   * never touches.
   */
  async listPrComments(pr: PrRef): Promise<PrComment[]> {
    const [issueComments, reviewComments, reviews] = await Promise.all([
      this.ghPaged<{ id: number; body: string | null; user: { login: string } | null; created_at: string }>(
        `/repos/${this.repo}/issues/${pr.number}/comments`,
        `listPrComments(#${pr.number}) issue comments`,
      ),
      this.ghPaged<{ id: number; body: string | null; user: { login: string } | null; created_at: string; path: string; line: number | null; original_line: number | null }>(
        `/repos/${this.repo}/pulls/${pr.number}/comments`,
        `listPrComments(#${pr.number}) review comments`,
      ),
      this.ghPaged<{ id: number; body: string | null; user: { login: string } | null; submitted_at: string | null; state: string }>(
        `/repos/${this.repo}/pulls/${pr.number}/reviews`,
        `listPrComments(#${pr.number}) reviews`,
      ),
    ]);
    const merged: PrComment[] = [
      ...issueComments
        .filter((c) => !MARKER_RE.test(c.body || ""))
        .map((c) => ({ id: `issue-${c.id}`, author: c.user?.login ?? "unknown", created_at: c.created_at, body: c.body || "" })),
      ...
      // A review's own body is often empty (e.g. a bare "Approve" click) —
      // skip those; the verdict without any text isn't a correction to fold
      // into a resumed prompt.
      ...reviews
        .filter((r) => (r.body || "").trim().length > 0)
        .map((r) => ({
          id: `review-${r.id}`,
          author: r.user?.login ?? "unknown",
          created_at: r.submitted_at ?? new Date(0).toISOString(),
          body: r.body || "",
          verdict: r.state,
        })),
    ];
