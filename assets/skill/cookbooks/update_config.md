...
Add or retune agents in `sf.config.yaml`.
...
**Resolution:** the agent's own list wins → else it inherits `defaults.tools` → else unset, meaning all tools. An empty list is not "all tools"; it is a tool-less agent, and it will stall.
...
3. **An output type.** Every agent call parses against a concrete zod-backed envelope type in `adw_modules/data_types.ts`. If none of `PlanOutput`, `BuildOutput`, `ScoutOutput`, `ReviewOutput`, `DocumentOutput` fits the new agent's report, add one — see `update_modules.md`. The user prompt's `Report` section must show exactly that JSON shape.
