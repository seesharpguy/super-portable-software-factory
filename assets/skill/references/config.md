...
No config toggle for this one — it's always on, the same way `blocked` itself always is. `<prefix>:feedback` is the build lane's own human-in-the-loop label: add it to an issue (its PR still open, or already declined) after leaving corrections as **comments on the PR**, and `spf watch` reruns that issue with the PR's comment thread folded into the prompt. A still-open PR gets updated in place (same branch, same PR, a new commit); a declined one gets a fresh `-rN` branch and a new PR that supersedes the old. Needs `CodeHostProvider.listPrComments` — implemented for both `github` and `bitbucket` — or the issue blocks again with an explanation instead of rebuilding blind. See the main README's "Revising a rejected (or still-open) PR" for the full state diagram.
