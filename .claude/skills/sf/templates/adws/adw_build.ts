...
import { BuildOutput, makeAgentCall, makePhaseParams } from "./adw_modules/data_types.ts";
...
if (import.meta.main) {
  ...
  if (positionals.length < 1) {
    console.error("usage: adw_build.ts <prompt or path/to/prompt.md> [--config <path>] [--adw-id <id>]");
    process.exit(1);
  }
