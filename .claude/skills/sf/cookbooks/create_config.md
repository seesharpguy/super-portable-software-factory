...
Generate `sf.config.yaml` — the agent roster for a target repo.

## Generate it

```bash
bun run .claude/skills/sf/scripts/make_config.ts
```

Writes `adws/adw_sf_config/sf.config.yaml` — creating the directory if needed — with the starter agents (planner, builder, scout, reviewer, documenter) wired to the prompt files `/sf install` stamped into `adws/adw_data/prompt_engineering/`. That path is the default every ADW and the justfile look for; `--config` overrides it. `make_config.ts` refuses to overwrite an existing config unless you pass `--force`, so retuning an existing roster is a hand edit — see `update_config.md`.
...
observability:
  db: adws/adw_data/sf.db          # tracer writes here; the UI polls it
