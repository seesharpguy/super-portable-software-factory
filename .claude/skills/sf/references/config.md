...
The full `sf.config.yaml` spec: every field, how defaults merge, and how model / thinking / tools / extensions map onto the coding agent.

It lives at **`adws/adw_sf_config/sf.config.yaml`** — the default path every `adw_*.ts` and the justfile resolve, and where `install.ts` / `make_config.ts` stamp it. Pass `--config <path>` to any ADW (or set `SF_CONFIG` for the justfile) to run against a different roster.
...
| `harness_engineering` | string[] | Coding-agent extensions. Pi: extension names. Claude Code: reserved (MCP, hooks). |
| `tools` | string[] | Roster-wide tool allowlist. Every agent that omits its own `tools` inherits this. Unset = all tools usable. |
| `protected_files` | string[] | Paths **no** agent may modify unless it names them in its own `writes`. Default: `adws/adw_modules/`, `adws/adw_sf_config/`, `adws/adw_*.ts` — an agent must not be able to edit the machinery that decides whether its work passed. |
...
| Field | Type | Meaning |
|---|---|---|
| `db` | path | SQLite trace db, read/written via `bun:sqlite`. `tracer.ts` writes it directly; the visualizer polls it. Default `adws/adw_data/sf.db`. |
...
```
model pattern 'gemini-3.6-flash' is ambiguous:
...
**Resolution order:** an agent's own `tools` list wins; an agent that omits the key inherits `defaults.tools`; if neither is set, `tools` stays unset and all tools are usable. An empty list is not "all tools" — it is a tool-less agent, and it will stall.
...
```yaml
defaults:
  protected_files: [adws/adw_modules/, adws/adw_sf_config/, "adws/adw_*.ts"]
