...
**Files are the raw record** (`raw_output.jsonl` streams, `envelope.json`, `agent_map.json`); **SQLite (`sf.db`, via `bun:sqlite`) is the queryable mirror** the UI reads. `tracer.ts` writes both. Losing the db loses nothing that can't be rebuilt from files.

Location comes from `observability.db` in `sf.config.yaml`, default `adws/adw_data/sf.db` — inside the **target** repo, gitignored.
...
| Type | Emitted when |
|---|---|
| `phase_start` | an `run.phase(...)` block is entered |
...
**Streaming is solved by construction.** `agent_pi.ts` tails pi's JSONL stdout chunk by chunk and the tracer inserts each event into `sf.db` **while the agent is still working** — never batched at phase end (verified in the first smoke run: tool calls visible mid-run). This is why the whole call chain from `agent_pi.run()` up through `agents.execute`, `run.phase`, and every ADW's `main()` is `async`: Bun/Node have no synchronous "iterate lines as they arrive" API, so streaming a live subprocess needs `async`/`await` all the way up. Everything downstream of the trace is a poll → render.
...
envelopes (
  ...
  agent         TEXT,
  output_type   TEXT,              -- name of the envelope type it parsed against
  payload_json  TEXT,
  valid         INTEGER,
  attempt       INTEGER,
  created_at    TEXT
